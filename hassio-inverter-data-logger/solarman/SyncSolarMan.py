...
class SyncSolarMan:
    ...
    def run(self):
        timeout = 3
        for res in socket.getaddrinfo(self.logger_ip, self.logger_port, socket.AF_INET,
                                      socket.SOCK_STREAM):
            family, socktype, proto, canonname, sockadress = res
            try:
                self.logger.info('connecting to {0} port {1}'.format(
                ...
                self.logger.error(
                    'Error connecting to logger with IP: {0} and SN {1}, trying next logger.'.format(self.logger_ip, self.logger_port))
                next = True
                break
        if (next):
            pass
